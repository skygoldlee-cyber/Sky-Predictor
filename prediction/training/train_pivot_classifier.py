"""Pivot Confirmation Classifier Training Script

후보 피봇 확정/취소 분류 모델 학습 스크립트.
...
def train_classifier(
    ...
    model = PivotConfirmationClassifier(input_dim=input_dim, hidden_dim=hidden_dim)
    model.to(device)
    ...
    for epoch in range(epochs):
        ...
        if val_metrics["loss"] < best_val_loss:
            ...
            checkpoint_path = output_path / "pivot_classifier_best.pt"
            torch.save({
                'epoch': epoch,
    ...
    # 최종 모델 저장
    final_path = output_path / "pivot_classifier_final.pt"
    torch.save({
        'model_state_dict': model.state_dict(),
        'input_dim': input_dim,
...
def main():
    parser = argparse.ArgumentParser(description="Pivot Confirmation Classifier Training")
