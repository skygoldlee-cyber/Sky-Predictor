# t8465 선물/옵션 분봉 데이터 수집 로직

## 개요

t8465는 eBest OpenAPI에서 선물/옵션의 분봉(또는 일봉) OHLCV 데이터를 가져오는 쿼리입니다. 본 문서는 KOSPI200 선물(KP200)과 KOSPI 지수 선물(KOSPI)의 분봉 데이터를 수집하는 로직을 설명합니다.

## 주요 컴포넌트

### 1. fetch_market_data_service.py

`t8465` 쿼리의 핵심 데이터 수집 로직이 구현된 서비스입니다.
...
```python
if query_type == "t8465" and bool(getattr(view, "use_replay", False)):
...
```python
if query_type == "t8465" and (not bool(getattr(view, "use_replay", False))):
...
```python
result = await self._fetch_market_data("t8465", symbol, date=date)
```
...
basic_market_data_service.get_basic_market_data()
    ↓
fetch_market_data("t8465", kp200_symbol, date)
    ↓
eBest API 요청 (t8465InBlock)
    ↓
응답 처리 (t8465OutBlock, t8465OutBlock1)
    ↓
데이터 변환 및 정규화
    ↓
Pivot 포인트 계산
...
option_aggregator.aggregate_symbol()
    ↓
fetch_market_data("t8465", option_symbol, date)
    ↓
...
- 옵션 종목의 t8465 요청 스킵 (빈 DataFrame 반환)
...
- `services/fetch_market_data_service.py` - t8465 핵심 수집 로직
