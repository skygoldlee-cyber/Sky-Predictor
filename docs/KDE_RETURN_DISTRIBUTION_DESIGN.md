...
#### 11.3.11 Random seed 고정 + 3회 반복 실험

결과 안정성을 확인하기 위해 다음 시드를 고정하고 파이프라인을 3회 반복 실행했습니다.

- `PYTHONHASHSEED=42`, `random.seed(42)`, `np.random.seed(42)`, `tf.random.set_seed(42)`
- XGBoost/RandomForest `random_state=42`

**3회 반복 결과 (test 2025-2026, look-ahead 배제)**

| Run | Variant | 거래 수 | 승률 | 총 PnL | 평균 PnL | Profit Factor | test Sharpe | MDD |
|-----|---------|--------|------|--------|---------|---------------|-------------|-----|
| 1 | Baseline (multi-tf) | 203 | 37.44% | -8,865,707 | -43,673 | 0.56 | -2.56 | 10,546,592 |
| 1 | **KDE (multi-tf)** | 166 | 48.80% | **+4,374,041** | **+26,350** | **1.50** | **1.59** | 2,628,650 |
| 2 | Baseline (multi-tf) | 243 | 41.15% | -8,211,277 | -33,791 | 0.70 | -1.91 | 9,838,796 |
| 2 | **KDE (multi-tf)** | 110 | 48.18% | **+1,766,985** | **+16,064** | **1.26** | **0.81** | 1,439,960 |
| 3 | Baseline (multi-tf) | 431 | 41.07% | -8,410,819 | -19,515 | 0.77 | -1.71 | 10,143,979 |
| 3 | **KDE (multi-tf)** | 273 | 45.79% | **+2,410,433** | **+8,829** | **1.18** | **0.78** | 2,913,605 |

**통계 요약**

| Variant | Sharpe 평균 | Sharpe 표준편차 | PnL 평균 | PnL 표준편차 | 거래 수 평균 |
|---------|------------|-----------------|----------|-------------|-------------|
| Baseline (multi-tf) | -2.06 | 0.44 | -8,495,934 | 335,415 | 292 |
| **KDE (multi-tf)** | **1.06** | **0.46** | **+2,850,486** | **1,358,094** | **183** |

- **3회 모두 KDE가 baseline을 능가**하고 양수 PnL을 기록.
