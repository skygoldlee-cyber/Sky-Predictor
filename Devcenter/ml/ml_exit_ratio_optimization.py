# -*- coding: utf-8 -*-
"""
청산 타이밍 최적화 (승/패 비율 개선)

손절/익절 비율 조정을 통해 승/패 비율을 개선한다.
"""
import sys
...
def optimize_stop_loss_take_profit(df: pd.DataFrame, stop_loss_pts: float, 
    ...
    """손절/익절 비율 조정"""
    # 손절/익절 필터링
    # 승리: net_pts >= take_profit_pts
    # 패배: net_pts <= -stop_loss_pts
    # 중간: 제외
    ...
    print(f"\n손절/익절 비율 조정 결과:")
    print(f"  손절라인: {stop_loss_pts} 포인트")
    print(f"  익절라인: {take_profit_pts} 포인트")
    print(f"  조정 전: {len(df)}건 (승률: {df['is_win'].mean() * 100:.2f}%)")
    print(f"  조정 후: {len(df_filtered)}건 (승률: {df_filtered['is_win'].mean() * 100:.2f}%)")
    
    return df_filtered


def test_multiple_ratios(df: pd.DataFrame):
    """다양한 손절/익절 비율 테스트"""
    print(f"\n{'='*100}")
    print("다양한 손절/익절 비율 테스트")
    print(f"{'='*100}")
    ...
    for stop_loss, take_profit in ratios:
        df_filtered = optimize_stop_loss_take_profit(df, stop_loss, take_profit)
    ...
    # 결과 출력
    print(f"\n{'손절':<10}{'익절':<10}{'거래 수':<15}{'승률':<15}{'총 PnL (원)':<20}{'승/패 비율':<15}")
    print(f"{'-'*100}")
    
    for result in results:
        print(f"{result['stop_loss']:<10}{result['take_profit']:<10}{result['n_trades']:<15}"
              f"{result['win_rate']:<15.2f}{result['total_pnl']:<20,.0f}{result['win_loss_ratio']:<15.4f}")
    
    return results


def select_optimal_ratio(df: pd.DataFrame, results: List[Dict]):
    """최적 손절/익절 비율 선택"""
    print(f"\n{'='*100}")
    print("최적 손절/익절 비율 선택")
    print(f"{'='*100}")
    
    # 승/패 비율이 1.5 이상이고 총 PnL이 가장 높은 조합 선택
    valid_results = [r for r in results if r['win_loss_ratio'] >= 1.5]
    
    if valid_results:
        optimal = max(valid_results, key=lambda x: x['total_pnl'])
    else:
        # 승/패 비율이 1.5 이상인 조합이 없으면 승/패 비율이 가장 높은 조합 선택
        optimal = max(results, key=lambda x: x['win_loss_ratio'])
    
    print(f"\n선택된 손절/익절 비율:")
    print(f"  손절라인: {optimal['stop_loss']} 포인트")
    print(f"  익절라인: {optimal['take_profit']} 포인트")
...
def compare_performance(df_original: pd.DataFrame, df_optimized: pd.DataFrame, 
                        stop_loss_pts: float, take_profit_pts: float):
    """성과 비교"""
    print(f"\n{'='*100}")
    print("성과 비교")
    print(f"{'='*100}")
...
def main():
    ...
    # 3) 다양한 손절/익절 비율 테스트
    results = test_multiple_ratios(df)
    
    # 4) 최적 손절/익절 비율 선택
    optimal = select_optimal_ratio(df, results)
    
    # 5) 최적 비율 적용
    df_optimized = optimize_stop_loss_take_profit(df, optimal['stop_loss'], optimal['take_profit'])
    
    # 6) 성과 비교
    compare_performance(df, df_optimized, optimal['stop_loss'], optimal['take_profit'])
    ...
    # 최적화된 데이터셋 저장
    optimized_path = OUTPUT_DIR / "exit_ratio_optimized_trades.csv"
