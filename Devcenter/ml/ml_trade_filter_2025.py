...
def train_xgboost_model_2025(df: pd.DataFrame) -> xgb.XGBClassifier:
    """2025년 데이터만 사용하여 XGBoost 모델 학습"""
    # 2025년 데이터만 선택
    df_2025 = df[df['year'] == 2025].copy()
    
    print(f"2025년 데이터: {len(df_2025)}건")
    print(f"2025년 승률: {df_2025['is_win'].mean() * 100:.2f}%")
    ...
    X = df_2025[feature_cols].copy().fillna(0).astype(float)
    y = df_2025['is_win'].copy()
    
    # 시간 기반 분할 (2025년 내에서 훈련/검증 분할)
    df_2025_sorted = df_2025.sort_values('entry_time').reset_index(drop=True)
    split_idx = int(len(df_2025_sorted) * 0.7)
...
def optimize_threshold_2025(df: pd.DataFrame, model: xgb.XGBClassifier) -> float:
    """2025년 데이터로 threshold 최적화"""
    df_2025 = df[df['year'] == 2025].copy()
    ...
    for threshold in np.arange(0.4, 0.9, 0.05):
        ...
        filtered_df = df_2025[filtered_mask].copy()
...
def main():
    """메인 함수"""
    print("=" * 80)
    print("2025년 특화 XGBoost 거래 필터링 모델")
    print("=" * 80)
    ...
    # 2025년 데이터만 사용하여 모델 학습
    model = train_xgboost_model_2025(df)
    
    # 2025년 데이터로 threshold 최적화
    best_threshold = optimize_threshold_2025(df, model)
    
    # 모델 저장
    model_path = MODELS_DIR / "trade_filter_xgboost_2025.json"
