...
def train_xgboost_model_2024(df: pd.DataFrame) -> xgb.XGBClassifier:
    """2024년 데이터만 사용하여 XGBoost 모델 학습"""
    # 2024년 데이터만 선택
    ...
    X = df_2024[feature_cols].copy().fillna(0).astype(float)
    y = df_2024['is_win'].copy()
    
    # 시간 기반 분할 (2024년 내에서 훈련/검증 분할)
    df_2024_sorted = df_2024.sort_values('entry_time').reset_index(drop=True)
    split_idx = int(len(df_2024_sorted) * 0.7)
...
def optimize_threshold_2024(df: pd.DataFrame, model: xgb.XGBClassifier) -> float:
    """2024년 데이터로 threshold 최적화"""
    df_2024 = df[df['year'] == 2024].copy()
    ...
    for threshold in np.arange(0.4, 0.9, 0.05):
        ...
        filtered_df = df_2024[filtered_mask].copy()
...
def main():
    """메인 함수"""
    print("=" * 80)
    print("2024년 특화 XGBoost 거래 필터링 모델")
    print("=" * 80)
    ...
    # 2024년 데이터만 사용하여 모델 학습
    model = train_xgboost_model_2024(df)
    
    # 2024년 데이터로 threshold 최적화
    best_threshold = optimize_threshold_2024(df, model)
    
    # 모델 저장
    model_path = MODELS_DIR / "trade_filter_xgboost_2024.json"
