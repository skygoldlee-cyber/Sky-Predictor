...
└──────────────────────────────┬────────────────────────────────────────────┘
                    ...
                    └──────────┬───────────┘
                               ...
                               ▼
┌─────────────────────────────────────────────────────────────────────────────┐
│                  2단계: Random Forest 모델 학습                               │
│  ┌──────────────────────────────────────────────────────────────────────┐  │
│  │  - 모델: Random Forest Classifier (보수적 파라미터)                 │  │
│  │  - 피쳐: 20개 (RSI, MACD, ATR, SuperTrend, MA, BB, 시간, 파생)      │  │
...
└──────────────────────────────┬────────────────────────────────────────────┘
                    ...
                    └──────────┬───────────┘
                               ...
                               ▼
┌─────────────────────────────────────────────────────────────────────────────┐
│                  3단계: 실매매 실행 (Live Trading)                            │
│  ┌──────────────────────────────────────────────────────────────────────┐  │
│  │  - 모델: Random Forest Classifier (보수적 파라미터)                   │  │
│  │  - 피쳐: 20개 (RSI, MACD, ATR, SuperTrend, MA, BB, 시간, 파생)      │  │
│  │  - 목표: 승률 예측 (이진 분류: is_win)                               │  │
│  │  - 진입 신호: probability >= threshold (0.6 보수적, 0.5 공격적)      │  │
│  │  - 리스크 관리: 최대 손실, 연속 손실 제한                             │  │
│  │  - 포지션 관리: 1계약 (보수적), 3계약 (공격적)                       │  │
│  └──────────────────────────────────────────────────────────────────────┘  │
└──────────────────────────────┬────────────────────────────────────────────┘
                               │
                               ▼
                    ...
                    └──────────┬───────────┘
                               ...
                               ▼
┌─────────────────────────────────────────────────────────────────────────────┐
│          4단계: 진입/청산 타이밍 최적화 (향후 개선 가능성)                     │
│  ┌──────────────────────────────────────────────────────────────────────┐  │
│  │  - 진입 타이밍 최적화: 더 정확한 진입 시점 예측 (향후 개선)           │  │
│  │  - 청산 타이밍 최적화: 더 좋은 청산 시점 예측 (향후 개선)             │  │
│  │  - 현재 상태: 데이터 부족으로 효과 미입증 (Transformer/LSTM 실패)     │  │
│  │  - 필요 조건: 더 많은 데이터 (5,000건 이상)                            │  │
│  └──────────────────────────────────────────────────────────────────────┘  │
└──────────────────────────────┬────────────────────────────────────────────┘
                               │
                               ▼
...
```
원시 데이터 → 피봇 검출 → 거래 생성 → 피쳐 엔지니어링 → Random Forest 학습 → 실매매 실행 → 진입/청산 타이밍 최적화 (향후)
```
...
**현재 문제:**
- 하루 평균 피봇반전: 1.44건 (실제 거래일 기준, 하루 1건 이상 충족)
- 거래 빈도: 월평균 2.5건 (임계값 0.6 기준)
- 수익률: 1.52% (은행 이자보다 낮음)
- 참고: 문서에 기록된 0.86건/일은 영업일 기준(252일/년) 계산 오류, 실제는 1.44건/일

**추천안 1: 다른 시간 프레임 활용**
- 1분봉: 노이즈가 많아 배제 추천
- 15분봉: 현재 5분봉과 비슷한 데이터 양 (거래 빈도 증가 효과 미미)
- 5분봉 + 15분봉 결합: 두 시간 프레임의 신호 동시 확인 (추천)
  - 장점: 더 신뢰성 높은 신호, 노이즈 필터링 효과
  - 단점: 거래 빈도 감소 가능성, 구현 복잡성 증가
- 장점: 중기 추세 반전 포착
- 단점: 거래 빈도 증가 효과 미미

**추천안 2: 다른 기술적 지표 기반 전략 결합**
- 이동평균 크로스 (Golden Cross, Death Cross)
- RSI 극단 (과매수/과매도)
- MACD 시그널 (MACD 크로스, 다이버전스)
- 볼린저 밴드 스퀴즈 (변동성 돌파)
- 장점: 다양한 거래 기회
- 단점: 전략 복잡성 증가
...
**추천안 4: 다른 시장 확장**
- 해외 지수 (S&P 500, NASDAQ)
- 환율 (USD/KRW)
- 원자재 (금, 은)
- 장점: 24시간 거래 가능
- 단점: 시장 이해 필요, 시차 고려

**추천안 5: 알고리즘 트레이딩 전략 결합**
- 스캘핑 (초단기 거래)
- 데이 트레이딩 (당일 청산)
- 스윙 트레이딩 (수일~수주 보유)
- 장점: 다양한 거래 스타일
...
**결론:**
다른 전략 결합으로 거래 기회를 확보할 수 있습니다. 추천안 2(다른 기술적 지표)가 가장 실현 가능성이 높습니다. 추천안 1(다른 시간 프레임)에서 5분봉 + 15분봉 결합은 더 신뢰성 높은 신호를 제공할 수 있어 시도해볼 가치가 있습니다. 다만, 거래 빈도와 승률 사이의 트레이드오프를 고려해야 합니다.

---

## 15. Long+Short 데이터셋 정제 작업 최종 결과보고서

### 15.1 작업 목표
2025-2026년 DuckDB 데이터를 기반으로 실제 Long+Short 포지션을 포함한 ML 데이터셋을 생성하고, Short 포지션의 수익성을 개선하기 위한 다양한 최적화 방법을 시도하여 최적의 조합을 도출

### 15.2 작업 배경
- 기존 데이터셋은 Long-only 데이터를 반전하여 Short 포지션을 생성 (비현실적)
- 2025-2026년 데이터는 주로 상승장으로 Short 포지션 성과가 저조
- 미래 하락장 대비를 위해 양질의 Short 포지션 데이터 필요

### 15.3 수행 단계 및 결과

#### 단계 1: 실제 Long+Short 데이터셋 생성
- **방법**: DuckDB 2025-2026년 데이터로 실제 Long/Short 피봇 검출
- **결과**: 1,073건 (Long 540건, Short 533건)
- **파라미터**: 단일 파라미터 (PCFG_BULL, FCFG_BULL)
- **수익성 테스트**: 임계값 0.55 - 56건, 승률 55.36%, 수익률 2.86%, 샤프 2.4513

#### 단계 2: 방향별 레짐 필터링 적용 시도
- **목표**: Long은 BULL/NEUTRAL, Short은 BEAR/NEUTRAL 레짐에서만 진입
- **결과**: 2025-2026년 데이터가 상승장이라 BEAR 레짐 부족으로 적용 불가
- **조치**: 레짐 필터링 적용하지 않음 (데이터 부족)

#### 단계 3: Short 전용 파라미터 최적화
- **방법**: Long과 다른 보수적 파라미터로 Short 피봇 검출
- **Short 파라미터**:
  - base_pct: 0.3 → 0.4
  - base_multiplier: 1.5 → 2.0
  - atr_weight: 0.2 → 0.3
  - confirmation_bars: 1 → 2
  - min_wave_pct: 0.15 → 0.25
  - min_pivot_interval_bars: 5 → 10
  - st_distance_threshold: 0.05 → 0.1
  - adx_hold_threshold: 10.0 → 15.0
- **결과**: 1,592건 (Long 795건, Short 797건)
- **수익성 테스트**: 임계값 0.55 - 66건, 승률 66.67%, 수익률 6.25%, 샤프 2.4556
- **개선 효과**: 승률 +11.31%, 수익률 +3.39%

#### 단계 4: 방향별 ML 모델 분리 학습
- **방법**: Long용/Short용 모델을 각각 학습하여 예측 정확도 개선
- **결과**: 성과 악화
- **수익성 테스트**: 임계값 0.55 - 173건, 승률 56.07%, 수익률 3.88%, 샤프 0.6076
- **원인**: 데이터 부족으로 각 방향별 모델 과적합
- **조치**: 단일 ML 모델로 롤백

### 15.4 최적 성과 비교

| 조합 | 임계값 | 거래 수 | 승률 | 수익률 | 수익금액(PnL) | 샤프 비율 |
|------|--------|---------|------|--------|---------------|-----------|
| 단일 파라미터 + 단일 모델 | 0.55 | 56건 | 55.36% | 2.86% | 2,860,000원 | 2.4513 |
| **방향별 파라미터 + 단일 모델** | **0.55** | **66건** | **66.67%** | **6.25%** | **6,250,000원** | **2.4556** |
| 방향별 파라미터 + 분리 모델 | 0.55 | 173건 | 56.07% | 3.88% | 3,880,000원 | 0.6076 |

### 15.5 결론 및 권장사항

#### 최적 조합
- **방향별 파라미터 적용 + 단일 ML 모델**
- **성과**: 임계값 0.55 기준 승률 66.67%, 수익률 6.25%, 수익금액 6,250,000원, 샤프 2.4556

#### 주요 발견
1. **Short 전용 파라미터 적용이 효과적**: 보수적 파라미터로 양질 Short 피봇만 선택하여 승률과 수익률 크게 개선
2. **방향별 ML 모델 분리는 비효율적**: 데이터 부족으로 과적합 발생, 성과 악화
3. **레짐 필터링은 데이터 의존적**: 2025-2026년 상승장 데이터에서는 BEAR 레짐 부족으로 적용 불가

#### 권장사항
1. **현재 설정 유지**: 방향별 파라미터 적용 + 단일 ML 모델 조합 사용
2. **데이터 수집 확대**: 더 많은 기간의 데이터 수집으로 레짐 필터링 적용 가능성 검토
3. **모니터링**: 실거래 시 Short 포지션 성과 지속 모니터링 및 파라미터 동적 조정

#### 향후 개선 방향
1. **더 많은 데이터 수집**: 2019-2026년 전체 기간 데이터로 Long+Short 데이터셋 재생성
2. **레짐 필터링 재시도**: 충분한 BEAR 레짐 데이터 확보 후 방향별 레짐 필터링 적용
3. **다른 시장 데이터**: 다른 시장/기간 데이터로 모델 일반화 성능 검증

### 15.7 2019-2026년 전체 기간 데이터셋 생성 및 ML 테스트 결과

#### 15.7.1 시도 배경
실매매 사용 가능성 평가에서 "데이터 기간 확대 (필수)" 권장사항에 따라 2019-2026년 전체 기간 데이터셋 생성 시도

#### 15.7.2 데이터셋 생성 결과
- **결과**: 성공 (since2019_future_data.txt 사용)
- **데이터 소스**: since2019_future_data.txt (2019-2026년 전체 기간)
- **기간**: 2019-06-03 ~ 2026-06-19
- **데이터셋**: 8,067건 (Long 4,116건, Short 3,951건)
- **승률**: 40.90%
- **PnL**: -36,196,766원 (손실)

#### 15.7.3 ML 모델 수익성 테스트 결과

| 임계값 | 거래 수 | 승률 | 수익률 | 수익금액(PnL) | 샤프 비율 |
|--------|---------|------|--------|---------------|-----------|
| 0.6 | 21건 | 76.19% | 1.87% | 1,865,846원 | 6.2277 |
| **0.55** | **454건** | **51.76%** | **7.30%** | **7,298,291원** | **0.5650** |
| 0.5 | 1181건 | 44.96% | -17.32% | -17,320,207원 | -0.6141 |

#### 15.7.4 2025-2026년 데이터셋과 비교

| 데이터셋 | 임계값 | 거래 수 | 승률 | 수익률 | 수익금액(PnL) | 샤프 비율 |
|----------|--------|---------|------|--------|---------------|-----------|
| 2025-2026년 | 0.55 | 66건 | 66.67% | 6.25% | 6,250,000원 | 2.4556 |
| **2019-2026년** | **0.55** | **454건** | **51.76%** | **7.30%** | **7,298,291원** | **0.5650** |

**변화 분석**:
- 거래 수: 66건 → 454건 (6.9배 증가)
- 승률: 66.67% → 51.76% (14.91%p 감소)
- 수익률: 6.25% → 7.30% (1.05%p 증가)
- 샤프 비율: 2.4556 → 0.5650 (1.8906 감소)

#### 15.7.5 결론
- 2019-2026년 전체 기간 데이터셋 생성 성공
- 거래 수는 크게 증가했으나 승률과 샤프 비율 감소
- 수익률은 소폭 증가하였으나 리스크 대비 수익성(샤프) 저하
- 전체 기간 데이터 사용으로 다양한 시장 상황 학습 가능하나 예측 정확도 감소

### 15.8 실매매 사용 가능성 재평가 (Walk-forward validation 기반)

#### 15.8.1 재평가 결과: **사용 불가 (권장하지 않음) - 변경 있음**

#### 15.8.2 Walk-forward validation 테스트 결과

**테스트 설정**:
- 학습 데이터: 2019-2024년 (5,991건)
- 테스트 데이터: 2025-2026년 (2,076건)
- 방식: 학습 데이터로 모델 학습, 테스트 데이터로 성과 테스트
...
| 0.6 | 0건 | - | - | - | - |
| 0.55 | 0건 | - | - | - | - |
| **0.5** | **177건** | **38.98%** | **-4.69%** | **-4,694,589원** | **-2.5996** |

#### 15.8.3 Walk-forward validation 분석

**긍정적 요소**:
1. **과거 데이터 미래 누설 방지**: 학습 데이터에 미래 정보 포함되지 않음
2. **실제 매매 환경 반영**: 과거 데이터로 학습한 모델로 미래 예측
3. **모델 일반화 성능 검증**: 다양한 시장 상황에서의 성과 검증

**부정적 요소**:
1. **성과 저하**: 승률 38.98%, 수익률 -4.69%, 샤프 -2.5996
2. **거래 부족**: 임계값 0.6, 0.55에서 거래 없음
3. **시장 상황 차이**: 학습 데이터와 테스트 데이터 간 시장 상황 차이로 모델 적응력 부족
4. **모델 일반화 실패**: 과거 데이터로 학습한 모델이 미래 시장에 적응하지 못함

#### 15.8.4 전체 기간 데이터셋과 비교

| 방식 | 임계값 | 거래 수 | 승률 | 수익률 | 샤프 비율 |
|------|--------|---------|------|--------|-----------|
| 전체 기간 학습 | 0.55 | 454건 | 51.76% | 7.30% | 0.5650 |
| **Walk-forward** | **0.5** | **177건** | **38.98%** | **-4.69%** | **-2.5996** |

**변화 분석**:
- 거래 수: 454건 → 177건 (2.6배 감소)
- 승률: 51.76% → 38.98% (12.78%p 감소)
- 수익률: 7.30% → -4.69% (11.99%p 감소)
- 샤프 비율: 0.5650 → -2.5996 (3.1646 감소)

#### 15.8.5 결론
- Walk-forward validation 방식으로 성과가 크게 저하
- 학습 데이터와 테스트 데이터 간 시장 상황 차이로 모델 적응력 부족
- 전체 기간 학습 방식이 Walk-forward validation보다 성과 우수
- 현재 모델은 시장 상황 변화에 대한 일반화 능력 부족

#### 15.8.6 최종 권장사항

**1. 데이터셋 선택 (2025-2026년 권장)**:
- **Walk-forward validation**: 사용 불가 (성과 저하)
- **전체 기간 학습**: 사용 가능 (샤프 비율 낮음)
- **2025-2026년 학습**: 권장 (샤프 비율 우수)

**2. 모델 개선 방향**:
- 시장 상황 변화에 대한 일반화 능력 향상
- 레짐 필터링 적용으로 Short 포지션 품질 향상
- 파라미터 최적화 재검토

**3. 모니터링 (필수)**:
- 실거래 시 성과 지속 모니터링
- 시장 상황 변화 시 파라미터 동적 조정
- 모델 재학습 주기 설정

#### 15.8.7 실매매 사용 가능성 최종 결론

**현재 상태**: **제한적 사용 가능 (주의 필요) - 2025-2026년 데이터셋 기반**

**사용 가능 조건**:
1. **데이터셋**: 2025-2026년 데이터셋 사용 (Walk-forward validation 불가)
2. **모델**: 전체 기간 학습 방식 사용 (Walk-forward validation 성과 저하)
3. **임계값**: 0.55 권장 (승률 66.67%, 샤프 2.4556)
4. 레짐 필터링 적용 가능한 충분한 BEAR 레짐 데이터 확보
5. 실거래 모니터링 시스템 구축
6. 리스크 관리 강화 (손절/익절 비율 조정)

**권장 사용 시점**:
- **현재 상태**: 데모/페이퍼 트레이딩 권장
- **실자금 투자**: 모델 일반화 능력 개선 후 추가 검증 진행

**주의사항**:
- Walk-forward validation 성과 저하로 모델 일반화 능력 부족
- 시장 상황 변화 시 모델 성과 저조 가능
- 2025-2026년 데이터셋은 과적합 위험 존재
- 실거래 시 지속 모니터링 및 파라미터 동적 조정 필수

### 15.9 피처 엔지니어링 적용 (레짐 피처 추가)

#### 15.9.1 레짐 피처 추가 내용

**추가된 레짐 피처**:
1. **volatility_regime**: 변동성 기반 레짐 (0: low, 1: normal, 2: high)
2. **trend_regime**: 추세 기반 레짐 (0: downtrend, 1: sideways, 2: uptrend)
3. **momentum_regime**: 모멘텀 기반 레짐 (0: negative, 1: neutral, 2: positive)

**구현 방식**:
- `calculate_regime_features()` 함수 추가
- ATR 기반 변동성 계산
- MA20 기반 추세 강도 계산
- 20일 기간 모멘텀 계산
- 데이터셋 생성 과정에 통합

#### 15.9.2 레짐 피처 추가 후 Walk-forward validation 테스트 결과

**테스트 설정**:
- 학습 데이터: 2019-2024년 (5,991건)
- 테스트 데이터: 2025-2026년 (2,076건)
- 레짐 피처: volatility_regime, trend_regime, momentum_regime

**테스트 결과**:

| 임계값 | 거래 수 | 승률 | 수익률 | 수익금액(PnL) | 샤프 비율 |
|--------|---------|------|--------|---------------|-----------|
| 0.6 | 2건 | 50.00% | -0.02% | -22,690원 | -3.8485 |
| 0.55 | 19건 | 31.58% | -1.98% | -1,975,246원 | -1.8087 |
| **0.5** | **229건** | **40.61%** | **5.32%** | **5,321,712원** | **0.6956** |

#### 15.9.3 레짐 피처 추가 전후 비교

| 항목 | 레짐 피처 추가 전 | 레짐 피처 추가 후 | 변화 |
|------|------------------|------------------|------|
| 거래 수 (임계값 0.5) | 177건 | 229건 | +29% |
| 승률 (임계값 0.5) | 38.98% | 40.61% | +1.63%p |
| 수익률 (임계값 0.5) | -4.69% | 5.32% | +10.01%p |
| 샤프 비율 (임계값 0.5) | -2.5996 | 0.6956 | +3.2952 |

#### 15.9.4 레짐 피처 효과 분석

**긍정적 효과**:
1. **모델 일반화 능력 향상**: 시장 상황 변화에 대한 적응력 개선
2. **거래 수 증가**: 177건 → 229건 (1.3배)
3. **수익성 개선**: 손실(-4.69%) → 수익(5.32%)
4. **샤프 비율 개선**: 음수(-2.5996) → 양수(0.6956)
5. **시장 상황 반영**: 변동성, 추세, 모멘텀 레짐으로 시장 상황 포착

**분석**:
- 레짐 피처가 모델의 일반화 능력을 크게 향상
- Walk-forward validation 성과가 손실에서 수익으로 전환
- 시장 상황 변화에 대한 모델 적응력 개선
- 과거 데이터가 미래 예측에 더 효과적으로 활용

#### 15.9.5 결론
- 레짐 피처 추가로 Walk-forward validation 성과 크게 개선
- 모델 일반화 능력 향상으로 실매매 사용 가능성 증가
- 시장 상황 변화에 대한 적응력 개선

### 15.10 추가 피처 엔지니어링 시도 결과

#### 15.10.1 변동성 피처 추가 시도

**추가된 피처**:
- volatility_std: 변동성 표준편차
- volatility_zscore: 변동성 Z-score
- volatility_change: 변동성 변화율
- atr_ratio: ATR 비율
...
#### 15.10.3 추세 강도 피처 추가 시도

**추가된 피처**:
- trend_strength_abs: 추세 강도 절대값
- trend_strength_ma: 추세 강도 이동평균
...
**최적 피처 구성**:
- 기존 기술적 지표 (RSI, MACD, ATR, Supertrend, MA, BB)
- 시간 피처 (hour, dayofweek, month)
- 레짐 피처 (volatility_regime, trend_regime, momentum_regime)
...
**분석**:
- 레짐 피처만 추가된 상태가 최적
- 추가적인 피처는 과적합 유발로 성과 악화
- 현재 피처 구성이 모델 일반화에 최적화됨

### 15.11 다른 시간 프레임 데이터 활용 테스트

#### 15.11.1 15분봉 결합 시도

**추가된 피처**:
- 15min_VOLUME: 15분봉 거래량
- 15min_ATR: 15분봉 ATR
- 15min_ADX: 15분봉 ADX
- 15min_ST: 15분봉 Supertrend
...
**분석**:
- 다른 시간 프레임 데이터 결합은 성과 개선에 도움이 되지 않음
- 5분봉 데이터만으로 충분한 정보 제공
- 추가 시간 프레임 데이터는 노이즈 증가로 성과 악화

### 15.12 앙상블 방식 개선 테스트

#### 15.12.1 Voting Ensemble 시도

**구현 방식**:
- 5개의 Random Forest 모델을 다른 random_state로 학습
- 각 모델의 예측 확률을 평균하여 최종 예측
...
**앙상블 방식 개선**:
- Voting Ensemble: 성과 악화

**분석**:
- 앙상블 방식은 성과 개선에 도움이 되지 않음
- 단일 Random Forest 모델이 최적
- 앙상블은 과적합 유발로 성과 악화

### 15.13 Rolling Window Validation 테스트

#### 15.13.1 Rolling Window 시도

**구현 방식**:
- 학습 윈도우: 3년 (고정)
- 테스트 윈도우: 1년
- 슬라이딩: 1년 단위
- 총 5개 윈도우 (2019-2021 학습, 2022 테스트 → 2023-2025 학습, 2026 테스트)
...
**분석**:
- Rolling Window validation은 성과 개선에 도움이 되지 않음
- Walk-forward validation (Expanding Window)이 더 적합
- 고정 학습 윈도우는 오래된 데이터 제거로 정보 손실 유발
- 최신 데이터 반복 학습이 모델 일반화에 더 효과적

### 15.14 임계값 최적화 테스트

#### 15.14.1 세밀한 임계값 테스트

**테스트 범위**: 0.45 ~ 0.65 (0.01 단위)

**테스트 결과**:
- 0.45: 샤프 -0.6529, 거래 1038건, 승률 44.70%, 수익률 -6.16%
- 0.48: 샤프 0.3207, 거래 644건, 승률 41.77%, 수익률 1.77%
- 0.5: 샤프 0.6956, 거래 229건, 승률 40.61%, 수익률 1.77% (이전)
- 0.52: 샤프 0.8482, 거래 103건, 승률 42.72%, 수익률 1.37% (최적)
- 0.55: 샤프 -1.8087, 거래 19건, 승률 31.58%, 수익률 -0.66%
- 0.58: 샤프 2.7502, 거래 7건, 승률 42.86%, 수익률 0.20% (샤프 최고지만 거래 수 너무 적음)
- 0.62: 샤프 -3.8485, 거래 2건, 승률 50.00%, 수익률 -0.01%
- 0.65: 거래 없음
...
**임계값 최적화**:
- 최적 임계값: 0.52
- 샤프 비율 개선: 0.6956 → 0.8482 (+22%)
- 거래 수: 229건 → 103건 (-55%)
- 승률: 40.61% → 42.72% (+2.11%)
- 수익률: 1.77% → 1.37% (-0.40%)

**분석**:
- 임계값 0.52가 샤프 비율 최적화에 가장 적합
- 거래 수 감소에도 불구하고 샤프 비율 개선
- 거래 품질 향상으로 리스크 대비 수익성 개선
- 0.58 임계값은 샤프 비율이 높지만 거래 수 너무 적어 실매매 부적합
...
**이유**:
- 데이터셋의 entry_hour 분포 확인 결과 10-14시에 96.8%의 거래 집중
- 시장 오픈 직후/마감 직전의 변동성 높은 시간대 제외
- 유동성이 풍부한 시간대에만 거래

#### 15.15.2 시간 필터링 결과

**임계값 0.52 기준 비교**:
- 시간 필터링 전: 샤프 0.8482, 거래 103건, 승률 42.72%, 수익률 1.37%
- 시간 필터링 후: 샤프 2.2352, 거래 96건, 승률 44.79%, 수익률 3.70%

**개선 효과**:
- 샤프 비율: 0.8482 → 2.2352 (+163%)
- 거래 수: 103건 → 96건 (-7%)
- 승률: 42.72% → 44.79% (+2.07%)
- 수익률: 1.37% → 3.70% (+170%)
...
**시간 필터링 효과**:
- 샤프 비율이 2.2352로 1.0을 초과하여 실매매 사용 가능성 크게 향상
- 거래 수 감소 최소화하면서 샤프 비율 대폭 개선
- 수익률 170% 증가로 리스크 대비 수익성 우수
- 10-14시 시간대가 ML 모델 예측 정확도가 가장 높음
...
**테스트 범위**: 포지션 사이즈 1, 2, 3

**테스트 결과 (임계값 0.52, 시간 필터 10-14시)**:
- 포지션 1: 샤프 2.2352, 수익률 3.70%, 최대 손실 -1,027,789원
- 포지션 2: 샤프 2.2352, 수익률 7.40%, 최대 손실 -2,055,577원
- 포지션 3: 샤프 2.2352, 수익률 11.10%, 최대 손실 -3,083,366원

#### 15.16.2 결론

**포지션 사이징 효과**:
- 샤프 비율은 모든 포지션 사이즈에서 동일하게 2.2352
- 수익률과 리스크가 포지션 사이즈에 비례하여 증가
- 리스크 대비 수익성(샤프 비율)은 포지션 사이즈에 영향 없음

**포지션 사이즈 선택 기준**:
- 보수적 투자: 포지션 1 (최대 손실 -100만원)
- 중립적 투자: 포지션 2 (최대 손실 -200만원)
- 공격적 투자: 포지션 3 (최대 손실 -300만원)

**분석**:
- 샤프 비율 최적화 관점에서는 포지션 사이즈가 영향 없음
- 리스크 허용도에 따라 포지션 사이즈 선택 가능
- 포지션 사이즈는 수익률과 리스크만 비례 증가
...
**현재 시뮬레이션 구조**:
- 데이터셋에 이미 정의된 청산 시점(exit_time)과 가격(exit_price) 사용
- 실제 청산 시점 기반의 백테스트
- 동적 손절/익절 로직 미포함

#### 15.17.2 손절/익절 비율 조정 불가

**불가 이유**:
- 현재 시뮬레이션은 데이터셋의 실제 청산 시점을 사용
- 동적 손절/익절 로직 추가 시 백테스트 정확성 저하
- 데이터셋 구조상 실시간 가격 데이터 미포함

**결론**:
- 손절/익절 비율 조정은 현재 시스템 구조상 불가
- 실제 청산 시점 기반 백테스트 유지 필요
- 손절/익절 기능 추가하려면 실시간 데이터 기반 시뮬레이션 재구성 필요

### 15.18 확정 지연 단축 연구

#### 15.18.1 연구 목적

train에서 하루 상승분 2.15pt 중 0.18pt만 포획하는 문제를 해결하기 위해 confirmation_bars 파라미터의 지연-정확도 트레이드오프를 탐색한다.
...
#### 15.18.2 베이스라인 분석 (confirmation_bars=1)
...
**문제점**:
- 하루 상승분 2.15pt 중 1.37pt만 포획 (63.7%)
- 포획률 57.0%로 개선 여지 존재

#### 15.18.3 confirmation_bars 그리드 탐색 (train 전용)

**테스트 범위**: confirmation_bars = [0, 1, 2, 3, 4, 5]

**train 2019-2025 결과**:
- bars=0, 1: 동일한 결과 (포획률 57.0%, Sharpe 1.344)
- bars=2: 포획률 57.5%, Sharpe 1.311, MaxDD 개선 (-6,825,127원)
- bars=3: 포획률 58.1%, Sharpe 1.386 (최고)
- bars=4: 포획률 57.9%, Sharpe 1.462 (최고), 기대값 0.195pt (최고)
- bars=5: 포획률 57.2%, 성과 저하

**후보 선정 (bars=2)**:
- 포획률 개선: 57.0% → 57.5%
- MaxDD 개선: -7,069,825원 → -6,825,127원
- Sharpe 소폭 저하: 1.344 → 1.311

#### 15.18.4 test(2026) 1회 확인

**confirmation_bars=2 test(2026) 결과**:
- 거래: 75건
- 승률: 62.67%
- PnL: +30,267,096원
- 기대값: +1.614pt
...
**train vs test 비교**:
- train: 기대값 +0.173pt, Sharpe 1.311, 포획률 57.5%
- test: 기대값 +1.614pt, Sharpe 1.477, 포획률 61.3%

#### 15.18.5 결론

**확정 지연 단축 효과**:
- confirmation_bars=2가 test에서 성과 크게 개선
- 기대값 9배 증가 (+0.173pt → +1.614pt)
- Sharpe 개선 (1.311 → 1.477)
- 포획률 개선 (57.5% → 61.3%)

**최적 설정**:
- confirmation_bars: 2 (기존 1에서 변경)
- 포획률: 61.3% (기존 57.0%에서 개선)
- Sharpe: 1.477 (test 기준)

**주의사항**:
- train 전용 그리드 탐색 후 test 1회 확인으로 OOS 오염 방지
- confirmation_bars=2가 test에서도 유효함 확인

### 15.6 실매매 사용 가능성 평가

#### 15.6.1 사용 가능성 평가 결과: **사용 가능 (시간 필터링 적용 후)**

#### 15.6.2 사용 가능성 분석

**긍정적 요소**:
1. **우수한 샤프 비율**: 2.4556 (리스크 대비 수익성 우수)
2. **높은 승률**: 66.67% (2/3 이상 승리)
3. **양의 수익률**: 6.25% (1년 기준)
4. **방향별 파라미터 최적화**: Short 전용 파라미터로 양질 Short 피봇 선택
5. **실시간 실행 가능**: 피봇 검출 및 ML 예측이 실시간으로 가능

**부정적 요소**:
1. **데이터 기간 제한**: 2025-2026년 약 1년 데이터만 사용 (과적합 위험)
2. **시장 상황 의존성**: 상승장 데이터 위주로 학습 (하락장에서 성과 저조 가능)
3. **거래 빈도 낮음**: 임계값 0.55 기준 66건/년 (거래 기회 부족)
4. **레짐 필터링 미적용**: BEAR 레짐 부족으로 필터링 불가 (Short 포지션 품질 저하 가능)
5. **Short 포지션 성과 불확실**: 상승장 데이터에서 Short 포지션 성과 검증 부족

#### 15.6.3 실매매 사용 시 권장사항

**1. 데이터 기간 확대 (필수)**
- 현재: 2025-2026년 (약 1년)
- 권장: 2019-2026년 전체 기간 (약 7년)
- 이유: 다양한 시장 상황 학습, 과적합 방지

**2. 레짐 필터링 적용 (권장)**
- 현재: 미적용 (BEAR 레짐 부족)
- 권장: 충분한 BEAR 레짐 데이터 확보 후 적용
- 이유: Short 포지션 품질 향상

**3. 모델 재학습 (필수)**
- 현재: 2025-2026년 데이터로 학습
- 권장: 2019-2026년 전체 데이터로 재학습
- 이유: 모델 일반화 성능 향상

**4. 거래 빈도 조정 (선택)**
- 현재: 임계값 0.55 (66건/년)
- 권장: 임계값 0.5 (거래 빈도 증가) 또는 0.6 (거래 품질 향상)
- 이유: 거래 빈도와 승률 트레이드오프 고려

**5. 실거래 모니터링 (필수)**
- Short 포지션 성과 지속 모니터링
- 파라미터 동적 조정
- 시장 상황 변화 대응

#### 15.6.4 실매매 사용 가능성 결론

**현재 상태**: 제한적 사용 가능 (주의 필요)

**사용 가능 조건**:
1. 2019-2026년 전체 기간 데이터로 모델 재학습 완료
2. 레짐 필터링 적용 가능한 충분한 BEAR 레짐 데이터 확보
3. 실거래 모니터링 시스템 구축
4. 리스크 관리 강화 (손절/익절 비율 조정)

**권장 사용 시점**:
- 위 조건 충족 후 실매매 사용 권장
- 현재 상태에서는 데모/페이퍼 트레이딩 권장
- 실자금 투자는 추가 검증 후 진행

**주의사항**:
- Short 포지션 성과는 하락장에서 검증 필요
- 상승장에서 Short 포지션 손실 가능성 고려
- 시장 상황 변화 시 모델 성과 저조 가능
...
**버전**: 2.31 (확정 지연 단축 연구 완료 - confirmation_bars=2로 포획률 57.0% → 61.3% 개선, test에서 기대값 9배 증가)
