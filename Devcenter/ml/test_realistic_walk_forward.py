# -*- coding: utf-8 -*-
"""
현실적 Walk-Forward Validation 테스트
2~3년 과거 데이터 기반
"""
...
def realistic_walk_forward_validation(window_years=2):
    """현실적 Walk-Forward Validation 테스트"""
    print("=" * 80)
    print(f"현실적 Walk-Forward Validation 테스트 (롤링 윈도우: {window_years}년)")
    print("=" * 80)
    ...
    for test_year in range(2021, 2027):
        # 훈련 데이터: test_year 이전 window_years 데이터
        train_start_year = test_year - window_years
        train_df = df[(df['year'] >= train_start_year) & (df['year'] < test_year)].copy()
        ...
        print(f"{'='*80}")
        print(f"훈련 데이터: {len(train_df)}건 ({train_start_year} ~ {test_year-1})")
    ...
    # 결과 요약
    print(f"\n{'='*80}")
    print(f"Walk-Forward Validation 결과 요약 (롤링 윈도우: {window_years}년)")
    print(f"{'='*80}")
...
if __name__ == "__main__":
    # 2년 롤링 윈도우 테스트
    results_2y, total_pnl_2y = realistic_walk_forward_validation(window_years=2)
    ...
    # 3년 롤링 윈도우 테스트
    results_3y, total_pnl_3y = realistic_walk_forward_validation(window_years=3)
