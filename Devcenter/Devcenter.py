...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        self.cipher_suite = Fernet(
            base64.urlsafe_b64encode(
                (self.mac_address + self.mac_address + self.mac_address)
                .encode()
                .ljust(32)[:32]
            )
        )
        ...
        self.backup_folder = (
            os.path.dirname(os.path.abspath(__file__)) + "\\Backup_devcenter"
        )
        ...
        # connect event
        self.api.on_message.connect(
            lambda api, msg: self.output_log(LOG_TYPE.MESSAGE, msg)
        )
        self.api.on_realtime.connect(
            lambda api, code, key, datas: self.output_log(
                LOG_TYPE.REALTIME, f"{code}, {key}, {datas}"
            )
        )
    ...
    def initUI(self):
        ...
        # is_darkMode = True
        print(f"is_darkMode: {is_darkMode}")
        ...
        for i in range(self.profile_count):
            ...
            profile_login.triggered.connect(
                lambda checked, i=i: self.func_profile_login(i)
            )
        ...
        for i in range(tool_action_count):
            ...
            action.triggered.connect(
                lambda checked, i=i: self.tool_action(tool_action_names[i])
            )

        menu_help = self.menuBar().addMenu(self.tr("&Help"))
        open_homepage = menu_help.addAction("LS증권 OpenApi 홈페이지")
        open_homepage.triggered.connect(
            lambda: webbrowser.open_new_tab("https://openapi.ls-sec.co.kr/intro")
        )
        ...
        self.tab_code_list.addTab(
            self.tree_sample_list, f"샘플목록 ({sum_samples_list})"
        )
        self.tab_code_list.setTabPosition(QTabWidget.TabPosition.South)
        if is_darkMode:
            self.tab_code_list.setStyleSheet(
                "QTabBar::tab {color: gray;} QTabBar::tab:selected {color: white;}"
            )
        else:
            self.tab_code_list.setStyleSheet(
                "QTabBar::tab {color: gray;} QTabBar::tab:selected {color: black;}"
            )
        ...
        if is_darkMode:
            self.tab_log_list.setStyleSheet(
                "QTabBar::tab {color: gray;} QTabBar::tab:selected {color: white;}"
            )
        else:
            self.tab_log_list.setStyleSheet(
        ...
        self.check_test_in_wrapping.checkStateChanged.connect(
            lambda state: self.text_test_request.setLineWrapMode(
        ...
        formlayout_test_request_settings.addRow(
            "tr_cont_yn", self.text_test_in_tr_cont_yn
        )
        formlayout_test_request_settings.addRow(
            "tr_cont_key", self.text_test_in_tr_cont_key
        )
        ...
        self.check_test_out_wrapping.checkStateChanged.connect(
            lambda state: self.text_test_response.setLineWrapMode(
                QPlainTextEdit.LineWrapMode.WidgetWidth
                if state == Qt.CheckState.Checked
                else QPlainTextEdit.LineWrapMode.NoWrap
            )
        ...
        formlayout_test_response_settings.addRow(
            "tr_cont_yn", self.text_test_out_tr_cont_yn
        )
        formlayout_test_response_settings.addRow(
            "tr_cont_key", self.text_test_out_tr_cont_key
        )
        formlayout_test_response_settings.addRow(
            "Response", self.check_test_out_wrapping
        )
    ...
    async def initalize_async(self):
        ...
        if os.path.exists(filepath):
            ...
            oauth_node = next(
                (x for x in root_groups if x.desc_name == "OAuth 인증"), None
            )
        ...
        if site_loaded:
            ...
            with open(filepath, "w", encoding="utf8") as f:
                ...
                for root_group in self.root_groups:
                    ...
                    for subgroup in root_group.sub_groupList:
                        lines.append(
                            f"1^{subgroup['id']}^{subgroup['name']}^{subgroup['accessUrl']}^{subgroup['protocolType']}\n"
                        )
                        for tr_prop in subgroup["tr_props"]:
                            lines.append(
                                f"2^{tr_prop['id']}^{tr_prop['trCode']}^{tr_prop['trName']}^{tr_prop['transactionPerSec']}\n"
                            )
                f.writelines(lines)
                f.close()
            pass
    ...
    async def set_code(self, ident: str):
        ...
        if isWebsocket:
            ...
            self.doc_desc.setText(
                f"{tr_cd}: {tr_name}, 초당: {tr_prop['transactionPerSec']}, path: {tr_path}"
            )
        ...
        if res_text is None:
            ...
            for field in fieldProps:
                ...
                if field["bodyType"] == "req_b":
                    if propertyLength is None:
                        ...
                        InblockRecord = BlockRecord(
                            field["propertyCd"].strip(),
                            LSSiteHelper.GetDataType(field["propertyType"]),
                            [],
                        )
                    else:
                        size = propertyLength.strip()
                        code = (
                            field["propertyCd"].replace("&nbsp;", "").replace("-", "")
                        )
            ...
            for block in blocks:
                ...
                table.add_rows(
                    [[x.code, x.name, x.var_type, x.size, x.desc] for x in block.fields]
                )
...
"""
                ...
                for block in inblocks:
                    ...
                    if is_occurs:
                        ...
                        for field in block.fields:
                            if field.var_type == "number":
                                src_text += (
                                    f"            '{field.code}': 0, # {field.name}\n"
                                )
                            else:
                                src_text += (
                                    f"            '{field.code}': '', # {field.name}\n"
                                )
                        src_text += "        },\n"
                src_text += "    }\n"
    ...
    async def set_macaddress(self) -> str:
        """MacAddress 설정"""
        text, ok = await QAsyncDialog.getText(
            self, "MacAddress설정", "법인경우 필수 세팅 (12자리)", self.mac_address
        )
        if ok:
        ...
        if action_name == "전체 URL":
            ...
            if self.cur_tr_prop is not None:
                ...
                for block in blocks:
                    ...
                    lines.append(
                        f"{block.name}_elements = [{', '.join([f'"{x.code}"' for x in block.fields])}]"
                    )
                    lines.append(
                        f"{block.name}_names = [{', '.join([f'"{x.name}"' for x in block.fields])}]"
                    )
                    lines.append("")
                self.text_result.setPlainText("\n".join(lines))
            else:
    ...
    async def func_profile_login(self, profile_index: int):
        ...
        try:
            ...
            appsecretkey = self.cipher_suite.decrypt(
                profile_obj["appsecretkey"].encode()
            ).decode()
        except:
            appkey = ""
            appsecretkey = ""
        ...
        if result == QDialog.DialogCode.Accepted:
            ...
            if remember:
                profile_obj["appkey"] = self.cipher_suite.encrypt(
                    appkey.encode()
                ).decode()
                profile_obj["appsecretkey"] = self.cipher_suite.encrypt(
                    appsecretkey.encode()
                ).decode()

            # print(f"appkey: {appkey}, appsecretkey: {appsecretkey}")
            self.output_log(LOG_TYPE.LOG, f"{name} 로그인중...")
            ok = await self.api.login(appkey, appsecretkey)
            if ok:
                self.output_log(
                    LOG_TYPE.LOG,
                    f"{name} 로그인성공: {"모의투자" if self.api.is_simulation else "실투자"}",
                )
            else:
                self.output_log(
                    LOG_TYPE.LOG, f"{name} 로그인실패: {self.api.last_message}"
                )
    ...
    async def func_run(self):
        ...
        if is_tr_request:
            ...
            if last_response is not None and last_response.tr_cd == tr_cd:
                ...
                self.text_test_request_time.setText(
                    f"요청시간: {datetime.fromtimestamp(last_response.request_time).strftime("%H:%M:%S.%f")[:-3]}"
                )
                self.text_test_response_time.setText(
                    f"응답시간(ms): {last_response.elapsed_ms}"
                )
        self.btn_run.setEnabled(True)
    ...
    async def func_test_request(self):
        ...
        response = await self.api.request(
            tr_cd, request_text, path=path, tr_cont=tr_cont_yn, tr_cont_key=tr_cont_key
        )
        if response is None:
        ...
        self.text_test_request_time.setText(
            f"요청시간: {datetime.fromtimestamp(response.request_time).strftime('%H:%M:%S.%f')[:-3]}"
        )
...
class LSSiteHelper:
    ...
    @staticmethod
    def initialize():
        LSSiteHelper.httpclient = aiohttp.ClientSession(
            timeout=aiohttp.ClientTimeout(total=10)
        )

    @staticmethod
    async def GetApiGuide():
        response = await LSSiteHelper.httpclient.get(
            LSSiteHelper.URL_BASE + "/apiservice"
        )
